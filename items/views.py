...
class item_info:
    def __init__(self, name, category, price, stock, id, cid):
        self.name = name
...
# Home page: show first 10 products and page count
def index(request):
    products = Product.objects.all()
    number_of_pages = math.ceil(Product.objects.count() / 10)
    products = products[:10]
    returnList = [
        item_info(i.name, i.category.name, i.price, i.stock, i.id, i.category.id)
        for i in products
    ]
    ...
    return render(
        request,
        "index.html",
        {"data": returnList, "pages": number_of_pages, "username": username},
    )
...
def addwishlist(request):
    ...
    if not request.user.is_authenticated:
        return JsonResponse({"in_stock": False, "error": "not_authenticated"})
    if product.stock > 0:
...
def view_by_category(request):
    ...
    if target_cat_id > 0:
        try:
            ...
            raise Http404
        returnList = [
            item_info(i.name, category.name, i.price, i.stock, i.id, i.category.id)
            for i in products
        ]
        username = request.user.username if request.user.is_authenticated else "Account"
        return render(
...
# Search products by keyword and paginate
def search(request):
    keyword = request.GET.get("keyword", "").strip()
    try:
        target_page = int(request.GET.get("page", "").strip())
    except (ValueError, TypeError):
        target_page = 1
    products = Product.objects.all()
    if keyword:
        products = products.filter(name__icontains=keyword)
        max_page_num = math.ceil(products.count() / 10)
        if target_page < 1 or not target_page:
            target_page = 1
        elif target_page > max_page_num:
            target_page = max_page_num
        From = (target_page - 1) * 10
        To = From + 10
        products = products[From:To]
    username = request.user.username if request.user.is_authenticated else "Account"
